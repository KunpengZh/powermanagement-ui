    ...
    <div style="margin-left: 20px; height: 100%">
        <el-card class="box-card" style="height: 100%;">
            ...
            <div style="display: flex; justify-content: flex-start">
                <el-select
                        size="small"
                        v-model="query.bankNames"
                        multiple
                        collapse-tags
                        style="margin-left: 20px;"
                        placeholder="请选择银行">
                    <el-option
                            v-for="item in bankNamesOptions"
                            :key="item.value"
                            :label="item.text"
                            :value="item.value">
                    </el-option>
    ...
    export default {
        ...
        methods: {
            handleExcelExport() {
                ...
                this.Utils.jsonToExcel('银行信息统计表.xlsx', '银行信息统计表', this.rowData, this.columnDefs,'银行信息统计表');
            },
            handleColDef() {
        ...
        beforeMount() {
            ...
            this.$http.get('/customerdata/getAllBankName').then(res => {
                res = res.body;
                let bankNamesOptions = [];
                res.forEach(function (bank) {
                    bankNamesOptions.push({
                        text: bank,
                        value: bank
                    })
                });
                this.bankNamesOptions = bankNamesOptions;
            });
        }
    };
</script>
